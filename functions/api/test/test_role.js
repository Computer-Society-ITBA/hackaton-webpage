...
const {roleMiddleware,ROLE_ADMIN, ROLE_JURY,ROLE_MENTOR, ROLE_USER} = require('../middleware/roleMiddleware')
// endpoints to test privilege middleware
router.get('/admin',authMiddleware, roleMiddleware([ROLE_ADMIN]),(req,res)=>{
...
router.get('/mentor',authMiddleware, roleMiddleware([ROLE_MENTOR]),(req,res)=>{
...
router.get('/admin_or_jury',authMiddleware, roleMiddleware([ROLE_ADMIN,ROLE_JURY]), (req,res)=>{
